...
export default function FeaturesSection() {
  return (
    <section className="section-pad bg-[#0a0a15]">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {WHY_US.map((item) => (
            <div key={item.title}
              ...
              <div className="feature-icon text-2xl shrink-0 group-hover:bg-indigo-500/25 transition-colors">
                {item.icon}
              </div>
              <div>
                <h3 className="font-semibold text-white text-sm font-display mb-1.5">{item.title}</h3>
