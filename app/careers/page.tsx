      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="glass rounded-2xl p-8 mb-14">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div>
              <h2 className="font-display font-bold text-2xl text-slate-900 mb-3">Life at OmniSphere 365</h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              {PERKS.map((p) => (
                <div key={p.title} className="bg-surface-subtle rounded-xl p-4 border border-surface-border">
                  <div className="text-2xl mb-1.5">{p.icon}</div>
                  <div className="text-slate-900 text-xs font-semibold">{p.title}</div>
        ...
        {/* Open roles */}
        <div className="mb-12">
          <h2 className="font-display font-bold text-2xl text-slate-900 mb-6">Open Positions</h2>
              ...
              <div key={role.title}
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-slate-900 font-semibold">{role.title}</h3>
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-brand-50 text-brand-700 border border-brand-100 font-medium">{role.dept}</span>
                  </div>
                  <div className="flex flex-wrap gap-3 text-xs text-slate-500">
                    ...
                    <span className="text-slate-400">{role.stack}</span>
                  </div>
                </div>
                <Link href={`mailto:shaikhjauwaad@gmail.com?subject=Application for ${role.title}`}
                  className="btn-secondary text-xs py-2 px-4 whitespace-nowrap group-hover:border-brand-300">
        ...
        {/* Internships */}
        <div className="mb-12">
          <h2 className="font-display font-bold text-xl text-slate-900 mb-5">Internships</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {internships.map((role) => (
              <div key={role.title} className="glass glass-hover rounded-xl p-5">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-slate-900 font-semibold text-sm">{role.title}</h3>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-success-50 text-success-600 border border-success-500/20 font-medium">Internship</span>
                </div>
        ...
        <div className="glass rounded-2xl p-7 text-center">
          ...
          <h3 className="font-display font-bold text-xl text-slate-900 mb-2">Don&apos;t See a Fit?</h3>
